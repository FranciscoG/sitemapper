var cheerio = require('cheerio');
var Bluebird = require('bluebird');
var url = require('url');
var deDupe = require('./dedupe.js');
var request = Bluebird.promisifyAll(require('request'));

var BaseUrl = "";

function isDef(x) {
  return typeof x !== 'undefined' && x !== null && x !== "";
}

/**
 * get all the download links, images, and pdfs on a page
 * @param {Object} $  Cheerio object. jquery-like $ object with document already loaded
 * @return {Object}
 * @return {Array} Object.links  Array of links or empty array
 * @return {Array} Object.images Array of images or empty array
 * @return {Array} Object.pdfs   Array of PDFs or empty array
 */
function getItems($){
  var result = { links : [], images : [], pdfs : [] };
  if (!$) { return result; }
  
  var links = [];
  var images = [];
  var pdfs = [];
  var current = "";

  $('a').each(function(i, elem){
    current = $(this).attr('href');
    if (isDef(current)) {
      if (current.indexOf('.pdf') > 0) {
        pdfs.push(current);
      } else {
        links.push(url.resolve(BaseUrl, current));
      }
    }
  });

  current = "";
  $('img').each(function(i, elem){
    current = $(this).attr('src');
    if (isDef(current)) {
      images.push(current);
    }
  });

  result.links = deDupe(links);
  result.images = deDupe(images);
  result.pdfs = deDupe(pdfs);
  return result;
}

/**
 * sends the request to a url
 * @param String siteURL
 * @return {Function} A Promise
 */
function Scraper(siteURL){
  //get the HTML for the page

  var _url = url.parse(siteURL);
  BaseUrl =  _url.protocol + '//' + _url.host;

  var options = {
    url: siteURL,
    headers: {
       'Host': _url.host
      }
  };

  var response;

  var req = request.getAsync(options)
    .then(function(args){
      if (args[0].statusCode === 200) {
        response = args[0];
        var html = args[1];
        var $ = cheerio.load(html);
        return $;
      } else {
        console.log(args[0].statusCode + " : " + args[0].statusMessage);
        return false;
      }
    })
    .then(getItems)
    .catch(function(e){
      console.error("error: " + e.message);
      process.exit(1);
    });


  return req.then(function(itemsObj){
      //reduce list to the number selected
      itemsObj.headers = response.headers;
      itemsObj.statusCode = response.statusCode;
      itemsObj.statusMessage = response.statusMessage;
      return itemsObj;
    });
}

module.exports = Scraper;